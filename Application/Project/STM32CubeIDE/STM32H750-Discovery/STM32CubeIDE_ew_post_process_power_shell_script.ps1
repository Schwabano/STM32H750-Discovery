...
{
  Write-Host eclipse project configuration file not found at path:
  Write-Host $Path
  Write-Host
  Read-Host Press Enter to continue | Out-Null
  [Environment]::Exit(6)
...
$includePathNode = ($xml.cproject.storageModule.cconfiguration.storageModule.configuration.folderInfo.toolChain.tool.option | where {$_.superClass -like "*compiler.option.include*" -and $_.valueType -eq "includePath"})
if ($includePathNode) {
...
$symbolsNode = ($xml.cproject.storageModule.cconfiguration.storageModule.configuration.folderInfo.toolChain.tool.option | where {$_.superClass -like "*compiler.option.definedsymbols*" -and $_.valueType -eq "definedSymbols"})
if ($symbolsNode) {
...
# Select libs node
$libsNode = ($xml.cproject.storageModule.cconfiguration.storageModule.configuration.folderInfo.toolChain.tool.option | where {$_.superClass -like "*linker*" -and $_.valueType -like "libs"})
if ($libsNode) {
...
$libPathNode = ($xml.cproject.storageModule.cconfiguration.storageModule.configuration.folderInfo.toolChain.tool.option | where {$_.superClass -like "*linker*" -and $_.valueType -like "libPaths"})
if ($libPathNode) {
