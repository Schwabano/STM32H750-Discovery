...
void* GfxCreateSurface( int aFormat, int aWidth, int aHeight )
{
  ...
  /* try to allocate the memory for the surface structure, the pixel buffer and the clut */
  surface = (XGfxSurface*)EwAlloc( sizeof( XGfxSurface ));
  pixelBuffer = EwAllocVideo( bitmapSize + clutSize );
  if ( accelClutSize > 0 )
    accelClut = EwAllocVideo( accelClutSize );
  if ( !surface || !pixelBuffer || (( accelClutSize > 0 ) && !accelClut ))
  {
    if ( surface )
      EwFree( surface );
    if ( pixelBuffer )
  ...
  if ( clutSize > 0 )
    surface->Clut         = (void*)((unsigned long)pixelBuffer + bitmapSize );
  if ( accelClutSize > 0 )
  {
    surface->AccelAddress = accelClut;
    surface->AccelClut    = (void*)(((unsigned long)accelClut + 3 ) &~ 3 );
  }
  ...
  XSurfaceMemory* aMemory )
{
  ...
  /* allocate memory only for the administration structure and clut */
  surface = (XGfxSurface*)EwAlloc( sizeof( XGfxSurface ));
  if ( accelClutSize > 0 )
    accelClut = EwAllocVideo( accelClutSize );
  if ( !surface || (( accelClutSize > 0 ) && !accelClut ))
  {
    if ( surface )
      EwFree( surface );
    if ( accelClut )
...
void GfxDestroySurface( void* aHandle )
{
  ...
  /* release the video memory of allocated clut */
  if ( surface->AccelAddress )
    EwFreeVideo( surface->AccelAddress );
